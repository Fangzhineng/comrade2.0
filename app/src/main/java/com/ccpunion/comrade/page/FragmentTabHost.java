package com.ccpunion.comrade.page;
...
public class FragmentTabHost extends TabHost implements
    ...
    public void setup(Context context, FragmentManager manager) {
        ensureHierarchy(context); // Ensure com.universal.frame.views required by super.setup()
        super.setup();
        mContext = context;
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args) {
        ...
        if (mAttached) {
            ...
            if (info.fragment == null) {
                ...
                ft.add(mContainerId, info.fragment);
                ft.commit();
//				return ;
            }
...
//				ft.detach(info.fragment);
        ...
        if (tabSpec.getTag().equals(texts[1]) || tabSpec.getTag().equals(texts[2]) || tabSpec.getTag().equals(texts[3])) {
            if (info.fragment == null) {
...
//					ft.detach(tab.fragment);
    ...
    private FragmentTransaction doTabChanged(String tabId,
        ...
        if (newTab == null) {
            throw new IllegalStateException("No tab known for taglv_tag " + tabId);
        }
        if (mLastTab != newTab) {
            if (ft == null) {
                ft = mFragmentManager.beginTransaction();
