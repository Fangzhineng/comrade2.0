...
public class PictureSelectorUtil {
    ...
    public static void createActivity(Activity context, int type, List<LocalMedia> list , int number) {
        // 进入相册 以下是例子：用不到的 api 可以不写
        PictureSelector.create(context)
                .openGallery(type)// 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                //.theme()// 主题样式设置 具体参考 values/styles   用法：R.style.picture.white.style
...
//                .videoMinSecond(10)
    ...
    public static void createFragment(Fragment context, int type, List<LocalMedia> list, int number) {
        // 进入相册 以下是例子：用不到的 api 可以不写
        PictureSelector.create(context)
                .openGallery(type)// 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
